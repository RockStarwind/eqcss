...
(function(root, factory) {
	...
	EQCSS.load = function() {
		...
		for (i = 0; i < script.length; i++) {
			...
			if (
				script[i].getAttribute("data-eqcss-read") === null &&
				script[i].type === "text/eqcss"
			) {
		...
		for (i = 0; i < link.length; i++) {
			...
			if (
				link[i].getAttribute("data-eqcss-read") === null &&
	...
	EQCSS.parse = function(code) {
		...
		code.replace(/(@element.*(?!@element))/g, function(string, query) {
			...
			query.replace(/(@element)\s*(".*?"|'.*?'|.*?)\s*(and\s*\(|{)/g, function(
				string,
				atrule,
				selector
			) {
	...
	EQCSS.apply = function() {
		...
		for (i = 0; i < EQCSS.data.length; i++) {
			...
			for (j = 0; j < elements.length; j++) {
				...
				relationship['element']['self'] = elements[j];
				...
				test_conditions: for (k = 0; k < EQCSS.data[i].conditions.length; k++) {
					...
					if (elements[j] != document.documentElement) {
						parent_computed_style = window.getComputedStyle(
					...
					if (EQCSS.data[i].conditions[k].unit === "vw") {
						...
						EQCSS.data[i].conditions[k].recomputed_value =
						...
						value = parseInt(EQCSS.data[i].conditions[k].value);
						EQCSS.data[i].conditions[k].recomputed_value =
							value * Math.min(window.innerWidth, window.innerHeight) / 100;
						...
						value = parseInt(EQCSS.data[i].conditions[k].value);
						EQCSS.data[i].conditions[k].recomputed_value =
							value * Math.max(window.innerWidth, window.innerHeight) / 100;
					} else if (
						EQCSS.data[i].conditions[k].unit != null &&
						...
						div.style.width =
							EQCSS.data[i].conditions[k].value + EQCSS.data[i].conditions[k].unit;
						...
						EQCSS.data[i].conditions[k].value = parseInt(
					...
					var final_value = recomputed
						? EQCSS.data[i].conditions[k].recomputed_value
						: parseInt(EQCSS.data[i].conditions[k].value);
					...
					switch (EQCSS.data[i].conditions[k].measure) {
						...
						case "min-width":
							...
							if (EQCSS.data[i].conditions[k].unit === "%") {
								element_width = parseInt(computed_style.getPropertyValue("width"));
								parent_width = parseInt(
									parent_computed_style.getPropertyValue("width")
								);
						...
						case "min-height":
							...
							if (EQCSS.data[i].conditions[k].unit === "%") {
								...
								parent_height = parseInt(
									parent_computed_style.getPropertyValue("height")
								);
						...
						case "min-scroll-x":
							...
							if (recomputed === true || EQCSS.data[i].conditions[k].unit === "px") {
								...
								if (
									elements[j] === document.documentElement ||
									elements[j] === document.body
								) {
								...
								if (
									!(
										element_scroll / (element_scroll_size - element_size) * 100 >=
										final_value
									)
								) {
									test = false;
						...
						case "min-lines":
							...
							if (element_line_height === "normal") {
								var element_font_size = parseInt(
									computed_style.getPropertyValue("font-size")
								);
						...
						case "min-aspect-ratio":
							...
							if (
								!(
									el_width / el_height <=
									elements[j].offsetWidth / elements[j].offsetHeight
								)
							) {
								test = false;
								break test_conditions;
				...
				if (test === true) {
					...
					css_code = css_code.replace(/eval\( *((".*?")|('.*?')) *\)/g, function(
						string,
						match
					) {
					...
					css_code = css_code.replace(
						...
						function(match, $1, $2) {
							switch ($2) {
								...
								case "emin":
									return (
										Math.min(elements[j].offsetWidth, elements[j].offsetHeight) /
											100 *
											$1 +
										"px"
									);
								...
								case "emax":
									return (
										Math.max(elements[j].offsetWidth, elements[j].offsetHeight) /
											100 *
											$1 +
										"px"
									);
